...
function removeSession(sessionID) {
  ...
  for (const childSessionID of childSessions.keys()) {
    const visited = new Set();
    let ancestorSessionID = childSessionID;
    while (childSessions.has(ancestorSessionID) && !visited.has(ancestorSessionID)) {
      visited.add(ancestorSessionID);
      ancestorSessionID = childSessions.get(ancestorSessionID);
      if (ancestorSessionID === sessionID) {
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (type === "session.deleted") {
        removeSession(sessionID ?? info?.id);
        return;
      }
      if (sessionID && childSessions.has(sessionID)) {
